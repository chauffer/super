...
class Astro:
    ...
    def __init__(self):
        ...
        self.help = "\n".join(["**.astro** <sign> [today|week|month|year] - Daily dose of bullshit",
    ...
    async def get_horoscope(self, msg, owo=False):
        ...
        when = fuz(
            msg[2] if len(msg) >= 3 else "today", self.times, "today"
        )
    ...
    async def chat(self, msg, owo):
        message = msg.split()
        if len(message) >= 2:
                return (await self.get_horoscope(message, owo))
        else:
            return ("Command Help", self.help)
